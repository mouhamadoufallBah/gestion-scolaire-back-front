...
class EnseignantController
{
    public function index()
    {
        if ($_SERVER['REQUEST_METHOD'] === "GET") {
            ...
            $user = new Moohamad\ApiGestionScolaireBack\Model\Enseignant($db);
    ...
    public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] === "POST") {
            ...
            if (!empty($data->nomComplet) && !empty($data->email) && !empty($data->password) && !empty($data->role_id) && $data->etat && !empty($data->matiere_id) && !empty($data->classe_id)) {
                ...
                $user->matiere_id = intval($data->matiere_id);
                ...
                if ($userConnected['role_id'] == 1) {
                    ...
                    if ($result) {
                        ...
                        echo json_encode(
                            ['message' => "L'ajout de Enseignant à echouer"]
                        );
                    }
                } else {
                    echo json_encode(
    ...
    public function update(int $id)
    {
        if ($_SERVER['REQUEST_METHOD'] === "PUT") {
            ...
            $data = json_decode(file_get_contents("php://input"));
            if ($id && !empty($data->nomComplet) && !empty($data->email) && !empty($data->password) && !empty($data->role_id) && !empty($data->etat) && !empty($data->matiere_id) && !empty($data->classe_id)) {
                $user->id = $id;
                $user->nomComplet = htmlspecialchars($data->nomComplet);
                $user->email = htmlspecialchars($data->email);
                $user->password = htmlspecialchars($data->password);
                ...
                if ($userConnected['role_id'] == 1) {
                    ...
                    if ($result) {
                        http_response_code(201);
                        echo json_encode(
                            ['message' => "Enseignant a été modifier avec succées"]
                        );
                    } else {
                        http_response_code(503);
                        echo json_encode(
                            ['message' => "La modifiaction du Enseignant à echouer"]
                        );
                    }
                } else {
                    echo json_encode(
    ...
    public function updateEtat(int $id)
    {
        if ($_SERVER['REQUEST_METHOD'] === "PUT") {
            ...
            $enseignant =$user->get($id)->fetch();
            if ($enseignant) {
                $user->id = $id;
                $user->etat = $enseignant['etat'] == "Actif" ? "Inactif" : "Actif";
            ...
            if ($id) {
                ...
                if ($userConnected['role_id'] == 1) {
                    ...
                    if ($result) {
                        http_response_code(200);
                        echo json_encode(
                            ['message' => "Enseignant supprimer avec succées"]
                        );
                    } else {
                        http_response_code(503);
                        echo json_encode(
                            ['message' => "La supression du Enseignant à echouer"]
                        );
                    }
                } else {
                    echo json_encode(
                ...
                echo json_encode(
                    ['message' => "Veuillez selectionner le Enseignant que vous voullez supprimer"]
                );
            }
        } else {
            http_response_code(405);
